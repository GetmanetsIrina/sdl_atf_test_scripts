...
local function Alert(params, self)
  ...
  local RespTimeSending = RespTimeout - 1000
  local RequestTime
  local RespTime
  ...
  :Do(function(_,data)
      ...
      RUN_AFTER(Resp, RespTimeSending)
    end)

  self.mobileSession1:ExpectResponse(cid, { success = true, resultCode = "SUCCESS" })
  :Timeout(RespTimeSending + 1000)
  :ValidIf(function()
      RespTime = timestamp()
      TimeBetweenReqRes = RespTime - RequestTime
      if TimeBetweenReqRes > RespTimeSending - 1000 and TimeBetweenReqRes < RespTimeSending + 1000 then
        return true
      else
        return false, "SDL triggers timeout earlier then expected("
          .. tostring(RespTimeSending) .." sec), after " .. tostring(TimeBetweenReqRes)
...
local function Slider(params, self)
  ...
  :ValidIf(function()
      ...
      if TimeBetweenReqRes > RespTimeSending - 1000 and TimeBetweenReqRes < RespTimeSending + 1000 then
        ...
        return false, "SDL triggers timeout earlier then expected(".. tostring(RespTimeSending) .." sec), after "
...
local function PerformInteraction(params, self)
  ...
  local RespTimeSending = RespTimeout + TimeToResponseForVR - 1000
  ...
  :Do(function(_,data)
      ...
      RUN_AFTER(RespUI, RespTimeSending)
    end)
  self.mobileSession1:ExpectResponse(cid, {success = true, resultCode = "SUCCESS"})
  :Timeout(RespTimeSending + 3000)
  :ValidIf(function()
      RespTime = timestamp()
      TimeBetweenReqRes = RespTime - RequestTime
      if TimeBetweenReqRes > RespTimeSending - 1000 and TimeBetweenReqRes < RespTimeSending + 1000 then
...
local function PerformInteractionVR(params, self)
  ...
  :Do(function(_,data)
      ...
      RUN_AFTER(RespVR, RespTimeSending)
    end)
  self.mobileSession1:ExpectResponse(cid, {success = true, resultCode = "SUCCESS"})
  :Timeout(RespTimeSending + 1000)
  :ValidIf(function()
