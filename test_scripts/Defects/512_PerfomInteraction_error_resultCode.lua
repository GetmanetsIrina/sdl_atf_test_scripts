---------------------------------------------------------------------------------------------------
-- User story: ttps://github.com/CustomSDL/Sync3.2v2/issues/512
---------------------------------------------------------------------------------------------------
...
local function PI_PerformViaVR_ONLY(paramsSend, self)
  ...
  self.mobileSession1:ExpectResponse(cid, { success = false, resultCode = "ABORTED", info = "Perform Interaction error response." })
...
local function PI_PerformViaBOTH(paramsSend, self)
  ...
  self.mobileSession1:ExpectResponse(cid, { success = false, resultCode = "TIMED_OUT", info = "Perform Interaction error response." })
  :ValidIf(function(_, data)
      if data.payload.triggerSource then
        return false, "SDL sends redundant triggerSource parameter in response"
      end
