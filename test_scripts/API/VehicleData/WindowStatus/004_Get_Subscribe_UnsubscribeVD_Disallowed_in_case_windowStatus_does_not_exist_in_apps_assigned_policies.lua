...
-- In case:
-- 1) RPCs(Get/Subscribe/UnsubscribeVehicleData) with the `windowStatus` param does not exist in app's assigned policies.
-- 2) App sends valid RPCs(Get/Subscribe/UnsubscribeVehicleData) requests with windowStatus=true to the SDL.
-- SDL does:
-- 1) send response RPCs(Get/Subscribe/UnsubscribeVehicleData) with (success:false, "DISALLOWED") to the mobile app.
---------------------------------------------------------------------------------------------------
...
common.Step("UnsubscribeVehicleData for windowStatus DISALLOWED", common.processRPCFailure, { "UnsubscribeVehicleData" })
