...
m.removeData = {
  ...
  UnsubscribeButton = function(pAppId, pTimes)
    if not pTimes then pTimes = 1 end
    m.getHMIConnection():ExpectRequest("Buttons.UnsubscribeButton", m.resumptionData[pAppId].subscribeButtons.Buttons)
    :Do(function(_,deleteData)
        m.sendResponse(deleteData)
      end)
    :Times(pTimes)
...
m.rpcsRevert = {
  ...
  subscribeButton = {
    Buttons = function(pAppId,pTimes)
      if not pTimes then pTimes = 1 end
      m.getHMIConnection():ExpectRequest("Buttons.SubscribeButton",m.resumptionData[pAppId].subscribeWayPoints.Navigation)
      :Do(function(_, data)
          m.sendResponse(data)
          m.removeData.UnsubscribeWayPoints(pAppId)
        end)
...
function m.subscribeButton(pAppId)
  ...
  m.getHMIConnection():ExpectRequest()
  :Do(function(_, data)
      m.getHMIConnection():SendResponse(data.id, data.method, "SUCCESS", {})
